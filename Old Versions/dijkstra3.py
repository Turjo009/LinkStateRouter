...
def dijkstra(graph, source, destination):
    ...
    # print(shortest_path_to_destination) 
    ...
    while node_list:
        ...
        for node in node_list:
            if (is_neighbour(graph, node, min_node) == True) and (node not in visited_list):
                if ((shortest_path_to_destination[min_node] + edge_cost(graph, min_node, node)) < shortest_path_to_destination[node]):
    ...
    return shortest_path_to_destination, predecessor_node
